...
@class QNAsyncUdpSendPacket;

NSString *const QNAsyncUdpSocketException = @"QNAsyncUdpSocketException";
NSString *const QNAsyncUdpSocketErrorDomain = @"QNAsyncUdpSocketErrorDomain";

NSString *const QNAsyncUdpSocketQueueName = @"QNAsyncUdpSocket";
NSString *const QNAsyncUdpSocketThreadName = @"QNAsyncUdpSocket-CFStream";

enum QNAsyncUdpSocketFlags
{
...
@interface QNAsyncUdpSocket ()
{
	...
	QNAsyncUdpSocketReceiveFilterBlock receiveFilterBlock;
	dispatch_queue_t receiveFilterQueue;
	BOOL receiveFilterAsync;
	
	QNAsyncUdpSocketSendFilterBlock sendFilterBlock;
	...
	QNAsyncUdpSendPacket *currentSend;
...
/**
 * The QNAsyncUdpSendPacket encompasses the instructions for a single send/write.
**/
@interface QNAsyncUdpSendPacket : NSObject {
@public
	NSData *buffer;
...
@implementation QNAsyncUdpSendPacket
...
@interface QNAsyncUdpSpecialPacket : NSObject {
@public
//	uint8_t type;
...
@implementation QNAsyncUdpSpecialPacket
...
@implementation QNAsyncUdpSocket
...
- (instancetype)initWithDelegate:(id<QNAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq
{
	LogTrace();
...
- (instancetype)initWithDelegate:(id<QNAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	LogTrace();
	...
	if ((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([QNAsyncUdpSocketQueueName UTF8String], NULL);
		}
...
- (id<QNAsyncUdpSocketDelegate>)delegate
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
		return delegate;
...
- (void)setDelegate:(id<QNAsyncUdpSocketDelegate>)newDelegate synchronously:(BOOL)synchronously
{
...
- (void)setDelegate:(id<QNAsyncUdpSocketDelegate>)newDelegate
{
	[self setDelegate:newDelegate synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<QNAsyncUdpSocketDelegate>)newDelegate
{
...
- (void)getDelegate:(id<QNAsyncUdpSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (void)setDelegate:(id<QNAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue synchronously:(BOOL)synchronously
{
	dispatch_block_t block = ^{
...
- (void)setDelegate:(id<QNAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self setDelegate:newDelegate delegateQueue:newDelegateQueue synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<QNAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
...
- (void)notifyDidConnectToAddress:(NSData *)anAddress
{
	...
	__strong id<QNAsyncUdpSocketDelegate> theDelegate = delegate;
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:QNAsyncUdpSocketErrorDomain
	                           code:QNAsyncUdpSocketBadConfigError
	                       userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:QNAsyncUdpSocketErrorDomain
	                           code:QNAsyncUdpSocketBadParamError
	                       userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"QNAsyncUdpSocketSendTimeoutError",
	                                                     @"QNAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Send operation timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:QNAsyncUdpSocketErrorDomain
	                           code:QNAsyncUdpSocketSendTimeoutError
	                       userInfo:userInfo];
}

- (NSError *)socketClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"QNAsyncUdpSocketClosedError",
	                                                     @"QNAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Socket closed", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:QNAsyncUdpSocketErrorDomain code:QNAsyncUdpSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:QNAsyncUdpSocketErrorDomain
	                           code:QNAsyncUdpSocketOtherError
	                       userInfo:userInfo];
}
...
- (BOOL)createSocket4:(BOOL)useIPv4 socket6:(BOOL)useIPv6 error:(NSError * __autoreleasing *)errPtr
{
	...
	int(^createSocket)(int) = ^int (int domain) {
         ...
         *  #535(QNAsyncUdpSocket can not send data when data is greater than 9K)
         *
         *
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		QNAsyncUdpSpecialPacket *packet = [[QNAsyncUdpSpecialPacket alloc] init];
...
- (void)maybeConnect
{
	...
	BOOL sendQueueReady = [currentSend isKindOfClass:[QNAsyncUdpSpecialPacket class]];
	
	if (sendQueueReady)
	{
		QNAsyncUdpSpecialPacket *connectPacket = (QNAsyncUdpSpecialPacket *)currentSend;
...
- (void)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	QNAsyncUdpSendPacket *packet = [[QNAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	QNAsyncUdpSendPacket *packet = [[QNAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
	packet->addressFamily = [QNAsyncUdpSocket familyFromAddress:remoteAddr];
...
- (void)setSendFilter:(QNAsyncUdpSocketSendFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue
{
	[self setSendFilter:filterBlock withQueue:filterQueue isAsynchronous:YES];
}

- (void)setSendFilter:(QNAsyncUdpSocketSendFilterBlock)filterBlock
            withQueue:(dispatch_queue_t)filterQueue
       isAsynchronous:(BOOL)isAsynchronous
{
	QNAsyncUdpSocketSendFilterBlock newFilterBlock = NULL;
...
- (void)maybeDequeueSend
{
	...
	if (currentSend == nil)
	{
		...
		while ([sendQueue count] > 0)
		{
			...
			if ([currentSend isKindOfClass:[QNAsyncUdpSpecialPacket class]])
			{
				[self maybeConnect];
...
- (void)doPreSend
{
	...
	if (sendFilterBlock && sendFilterQueue)
	{
		...
		if (sendFilterAsync)
		{
			...
			QNAsyncUdpSendPacket *sendPacket = currentSend;
...
- (void)setReceiveFilter:(QNAsyncUdpSocketReceiveFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue
{
	[self setReceiveFilter:filterBlock withQueue:filterQueue isAsynchronous:YES];
}

- (void)setReceiveFilter:(QNAsyncUdpSocketReceiveFilterBlock)filterBlock
               withQueue:(dispatch_queue_t)filterQueue
          isAsynchronous:(BOOL)isAsynchronous
{
	QNAsyncUdpSocketReceiveFilterBlock newFilterBlock = NULL;
	dispatch_queue_t newFilterQueue = NULL;
...
+ (void)listenerThread:(id)unused
{
	@autoreleasepool {
		...
		[[NSThread currentThread] setName:QNAsyncUdpSocketThreadName];
...
+ (void)addStreamListener:(QNAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(QNAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	@autoreleasepool {
		QNAsyncUdpSocket *asyncUdpSocket = (__bridge QNAsyncUdpSocket *)pInfo;
